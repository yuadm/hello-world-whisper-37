...
const handleStatusCardClick = (status: 'compliant' | 'overdue' | 'due' | 'pending') => {
  ...
  // Helper function to generate PDF blob for spot check
  const generateSpotCheckPdfBlob = async (data: any, employeeName: string, companyInfo: any): Promise<Blob> => {
    // Import PDF generation functions to get the actual PDF document
    ...
    // Import fonts
    const DejaVuSansRegularUrl = (await import('@/assets/fonts/dejavu/DejaVuSans.ttf')).default;
    const DejaVuSansBoldUrl = (await import('@/assets/fonts/dejavu/DejaVuSans-Bold.ttf')).default;
    ...
    const fields = [
      `Service User: ${data.serviceUserName || 'N/A'}`,
      `Date: ${data.date || 'N/A'}`,
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Preparing all compliance forms for download.",
      });

      // Process all compliance records for this type
      for (const item of employeeStatusList) {
        if (item.record && item.record.completion_method && item.record.status === 'completed') {
          try {
            let pdfBlob: Blob | null = null;
            let fileName = '';

            switch (item.record.completion_method) {
              case 'spot_check':
                if (item.record.notes) {
                  try {
                    const spotCheckData = JSON.parse(item.record.notes);
                    pdfBlob = await generateSpotCheckPdfBlob(spotCheckData, item.employee.name, {
                      name: companySettings?.name || 'Company',
                      logo: companySettings?.logo
                    });
                    fileName = `${item.employee.name.replace(/[^a-z0-9]/gi, '_')}_SpotCheck_${item.record.period_identifier}.pdf`;
                  } catch (error) {
                    console.error('Error generating spot check PDF:', error);
                  }
                }
                break;
              ...
              case 'supervision':
                if (item.record.notes) {
                  try {
                    const supervisionData = JSON.parse(item.record.notes);
                    // For now, create a simple text-based PDF for supervision
                    const { PDFDocument, rgb } = await import('pdf-lib');
                    const doc = await PDFDocument.create();
                    const page = doc.addPage();
                    
                    page.drawText('Supervision Form', {
                      x: 50,
                      y: 750,
                      size: 16,
                    });
                    
                    page.drawText(`Employee: ${item.employee.name}`, {
                      x: 50,
                      y: 720,
                      size: 12,
                    });
                    ...
                    fileName = `${item.employee.name.replace(/[^a-z0-9]/gi, '_')}_Supervision_${item.record.period_identifier}.pdf`;
                  } catch (error) {
                    console.error('Error generating supervision PDF:', error);
                  }
                }
                break;
              ...
              case 'annual_appraisal':
                if (item.record.notes) {
                  try {
                    const appraisalData = JSON.parse(item.record.notes);
                    // For now, create a simple text-based PDF for annual appraisal
                    const { PDFDocument, rgb } = await import('pdf-lib');
                    const doc = await PDFDocument.create();
                    const page = doc.addPage();
                    
                    page.drawText('Annual Appraisal Form', {
                      x: 50,
                      y: 750,
                      size: 16,
                    });
                    ...
                    const pdfBytes = await doc.save();
                    pdfBlob = new Blob([pdfBytes], { type: 'application/pdf' });
                    fileName = `${item.employee.name.replace(/[^a-z0-9]/gi, '_')}_AnnualAppraisal_${item.record.period_identifier}.pdf`;
                  } catch (error) {
                    console.error('Error generating annual appraisal PDF:', error);
                  }
                }
                break;
            }
            ...
            console.error(`Error processing record for ${item.employee.name}:`, error);
          }
        }
      }
      ...
      if (fileCount === 0) {
        toast({
          ...
          description: "No completed compliance forms found for this compliance type.",
          variant: "destructive",
        });
        return;
      }
      ...
      const zipFileName = `${complianceType.name.replace(/[^a-z0-9]/gi, '_')}_ComplianceForms_${new Date().toISOString().split('T')[0]}.zip`;
      saveAs(zipBlob, zipFileName);

      toast({
        title: "Download complete",
        description: `Downloaded ${fileCount} compliance forms in ${zipFileName}`,
      });
      ...
      toast({
        ...
        description: "There was an error creating the download file.",
        variant: "destructive",
      });
    }
  };
